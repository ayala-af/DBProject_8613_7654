...
SELECT *
FROM teachers t1 
JOIN teach t2 ON t1.teacher_id = t2.teacher_id 
WHERE workshop_id = &<name="workshop" 
                      list="select workshop_id,workshop_name from workshops order by workshop_id"
                      description="yes" 
                      restricted="yes" >
      and extract(year from To_DATE(teacher_join_date,'DD/MM/YYYY')) <= &<name=join_date 
                                                                          type=integer 
                                                                          hint="enter a year (4 digit number)">; 
 
--params2
-- find all clients who participate in more than a specified number of workshops
SELECT c.client_ID, (c.client_fname || ' ' || c.client_lname) AS client_name, COUNT(DISTINCT sc.workshop_ID) AS workshop_count
FROM Clients c
...
select workshop_id,workshop_name,teacher_id,(teacher_fname || ' ' || teacher_lname) AS teacher_name,
       group_id,room_id,room_floor,isstudio,room_capacity
from teachers 
natural join scheduler 
natural join workshops 
natural join rooms
where sc_day = &<name="day" 
                 list="1,2,3,4,5,6" 
                 type="integer" 
                 default=1> 
      and sc_time = &<name="hour"
                      type=integer 
                      hint="a number between 6 and 24"
                      default=6>;


--params4
-- find clients living in the same city as the teacher with the specified teacher_id
SELECT c.client_ID, (c.client_fname || ' ' || c.client_lname) AS client_name, ci.city_name
FROM Clients c
